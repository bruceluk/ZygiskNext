...
// Global variables
vector<tuple<dev_t, ino_t, const char *, void **>> *inline_hook_list;
...
DCL_HOOK_FUNC(int, pthread_attr_destroy, void *target) {
    ...
    LOGI("pthread_attr_destroy called\n");
    // Only perform unloading on the main thread
    if (gettid() != getpid())
        return res;

    LOGI("pthread_attr_destroy\n");
...
static void hook_register(dev_t dev, ino_t inode, const char *symbol, void *new_func, void **old_func) {
    ...
    inline_hook_list->emplace_back(dev, inode, symbol, old_func);
}

#define HOOK_REGISTER(LIB, SYM, FUNC) \
    LOGE("hook %s:%s\n", LIB, SYM); \
    stub_##FUNC = shadowhook_hook_sym_name(LIB, SYM, (void*) new_##FUNC, (void **) &old_##FUNC); \
    if(stub_##FUNC == NULL) \
    { \
        ...
        LOGE("hook success %s:%s %x\n", LIB, SYM, stub_##FUNC); \
    }

void hook_functions() {
    // default_new(inline_hook_list);
    // default_new(jni_hook_list);

    shadowhook_init(SHADOWHOOK_MODE_SHARED, true);
    HOOK_REGISTER(LIB_LIBC, "fork", fork);
    HOOK_REGISTER(LIB_LIBC, "unshare", unshare);
    HOOK_REGISTER(LIB_LIBC, "strdup", strdup);
    HOOK_REGISTER(LIB_LOG, "__android_log_close", android_log_close);

    // Remove unhooked methods
    // inline_hook_list->erase(
    //         std::remove_if(inline_hook_list->begin(), inline_hook_list->end(),
    //                        [](auto &t) { return *std::get<3>(t) == nullptr;}),
    //         inline_hook_list->end());
}

static void hook_unloader() {
    HOOK_REGISTER(LIB_LIBC, "pthread_attr_destroy", pthread_attr_destroy);
}

static void unhook_functions() {
    // Unhook plt_hook
    for (const auto &[dev, inode, sym, old_func] : *inline_hook_list) {
        if (!lsplt::RegisterHook(dev, inode, sym, *old_func, nullptr)) {
            LOGE("Failed to register plt_hook [%s]\n", sym);
        }
    }
    delete inline_hook_list;
}
